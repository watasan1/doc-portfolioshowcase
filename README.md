...
- コンテンツ一覧を表示
- トピックボタンでフィルタリング可能
- 各コンテンツをクリックして詳細ページへ遷移

### コンテンツ詳細ページ

- README.mdの内容をレンダリング表示
- URL構造はGitHub と同じ「Domain.com/オーナー名/リポジトリ名」
- GitHubのリポジトリへの外部リンクへのボタンをクリックして、 のリポジトリ・デモサイトに遷移できる
...
2. あるファイルが別のファイルを必要としている場合、自動で正しい順番にまとめる
3. 最適化（圧縮・トランスパイル）
   * 不要なコードの削除や、古いブラウザ向けに変換
